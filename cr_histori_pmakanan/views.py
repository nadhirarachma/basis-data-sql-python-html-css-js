    ...
    if request.session.has_key('email'):
        ...
        try:
            if (request.session['role'] == ['admin']):
                ...
                # cursor.execute("SET SEARCH_PATH TO HIDAY")
                cursor.execute("SELECT id, nama FROM hiday.PRODUK WHERE id LIKE'PM%'")
                result = tupleFetch(cursor)
                ...
                if (form.is_valid and request.method == 'POST'):
                    ...
                    cursor.execute("SELECT jumlah, id_produk FROM hiday.PRODUK_DIBUTUHKAN_OLEH_PRODUK_MAKANAN WHERE id_produk_makanan = '" + idproduk + "'")
                    ...
                    cursor.execute("SELECT id_produk, jumlah FROM hiday.LUMBUNG_MEMILIKI_PRODUK WHERE id_lumbung = '" + email + "'")
                    result = tupleFetch(cursor)
                    ...
                    cursor.execute("SELECT id_alat_produksi FROM hiday.PRODUKSI WHERE id_produk_makanan = '" + idproduk + "'")
                    result = cursor.fetchone()

                    alatproduksi = result[0]
                    # print(alatproduksi)

                    cursor.execute("SELECT id_koleksi_aset FROM hiday.KOLEKSI_ASET_MEMILIKI_ASET WHERE id_aset = '" + alatproduksi + "'")
                    result = tupleFetch(cursor)
                    # print(result)
